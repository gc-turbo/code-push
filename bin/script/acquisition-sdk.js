...
var AcquisitionManager = /** @class */ (function () {
    function AcquisitionManager(httpRequester, configuration) {
        this._serverUrl = "https://us-central1-codepush-like.cloudfunctions.net/";
    ...
    AcquisitionManager.prototype.queryUpdateWithCurrentPackage = function (currentPackage, callback) {
        ...
        var safeAppVersion = currentPackage.appVersion.replace(/\./g, "_");
        // TODO: dev VS prod
        var requestUrl = "https://ota-updates-dev.s3.amazonaws.com/" + this._deploymentKey + "_v-" + safeAppVersion + ".json";
        console.log("update_check request URL: " + requestUrl + " and params: " + JSON.stringify(updateRequest));
        this._httpRequester.request(0 /* GET */, requestUrl, function (error, response) {
            ...
            if (response.statusCode !== 200) {
                ...
                callback(new code_push_error_1.CodePushHttpError(errorMessage), 
                /*remotePackage=*/ null);
                return;
            }
            console.log("update_check response: " + response.body);
            try {
            ...
            else if (updateInfo.package_hash === updateRequest.package_hash) {
                callback(/*error=*/ null, /*remotePackage=*/ null);
                return;
            }
            var remotePackage = {
        ...
        if (deployedPackage) {
            ...
            switch (status) {
                ...
                default:
                    if (callback) {
                        if (!status) {
                            callback(new code_push_error_1.CodePushDeployStatusError("Missing status argument."), 
                            /*not used*/ null);
                        }
                        else {
                            callback(new code_push_error_1.CodePushDeployStatusError('Unrecognized status "' + status + '".'), 
                            /*not used*/ null);
                        }
                    }
                    return;
